...
func (r *AutoscalingRunnerSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	default:
		...
		if integrityDiff {
			...
			// leave the listener, ERS, and running pods untouched until all jobs have drained.
			...
			if activeRunners > 0 {
				log.Info("Ephemeral runner set spec changed but runners are still active; waiting for runners to drain")
